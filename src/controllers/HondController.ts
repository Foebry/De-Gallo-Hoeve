...
const save = async (klant: IsKlantCollection, hond: HondCollection): Promise<HondCollection> => {
  const klantCollection = await getKlantCollection();
  const { modifiedCount } = await klantCollection.updateOne({ _id: klant._id }, { $addToSet: { honden: hond } });
...
export const getHondenByKlantId = async (klant_id: ObjectId): Promise<HondCollection[]> => {
  const klant = await getKlantById(klant_id);
  return klant?.honden.filter((hond) => !hond.deleted_at) ?? [];
};

export const getKlantHond = async (klant_id: ObjectId, hond_id: ObjectId): Promise<HondCollection | null> => {
  const klant = await getKlantById(klant_id);
  if (!klant) return null;
  const hond = klant.honden.find((hond) => hond._id.toString() === hond_id.toString() && !hond.deleted_at);
  return hond ?? null;
};

const update = async (klant: IsKlantCollection, hond_id: ObjectId, data: HondCollection): Promise<HondCollection> => {
  const klantCollection = await getKlantCollection();
...
const hardDelete = async (klant: IsKlantCollection, hond: HondCollection): Promise<void> => {
  const klantCollection = await getKlantCollection();

  const { modifiedCount } = await klantCollection.updateOne({ _id: klant._id }, { $pull: { honden: hond } });
  if (modifiedCount !== 1) throw new InternalServerError();
};

const softDelete = async (klant: IsKlantCollection, hond: HondCollection): Promise<void> => {
  const klantCollection = await getKlantCollection();
...
export const getHondById = async (_id: ObjectId): Promise<HondCollection | null> => {
  ...
  return klant?.honden.find((hond) => hond._id.toString() === _id.toString() && !hond.deleted_at) ?? null;
};
...
export type IsHondController = {
  ...
  update: (klant: IsKlantCollection, _id: ObjectId, hondData: HondCollection) => Promise<HondCollection>;
