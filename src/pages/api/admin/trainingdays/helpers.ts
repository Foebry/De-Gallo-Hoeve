...
import { notEmpty, toReadableDate } from 'src/shared/functions';
...
export const getDifferenceInTrainingDays = async (
  ...
  const hasDateBeenRemoved = (day: TrainingDaysCollection) =>
    !selected.map((newDay: TrainingDayDto) => newDay.date.split('T')[0]).includes(day.date.toISOString().split('T')[0]);
  ...
  const daysToAdd = selected.filter(
    (newDay: TrainingDayDto) =>
      !currentTrainingDays.map((day) => day.date.toISOString().split('T')[0]).includes(newDay.date.split('T')[0])
  );
  ...
  const daysToUpdate = currentTrainingDays
    ...
    .filter(
      (curr) =>
        !daysToAdd.map((day: TrainingDayDto) => day.date.split('T')[0]).includes(curr.date.toISOString().split('T')[0])
    )
    .map((curr) => ({
      ...curr,
      timeslots:
        selected.find((dto: TrainingDayDto) => dto.date.split('T')[0] === curr.date.toISOString().split('T')[0])
          ?.timeslots ?? [],
    }));
  ...
  const moreDates = daysToUpdate
    .map((trainingDay) => {
      ...
      const missingTimeslots = currentTrainingDays
        .find((currentTrainingDay) => currentTrainingDay.date.toISOString() === trainingDay.date.toISOString())
...
export const orderInschrijvingenToDeleteByKlantId = async (
  ...
  const orderedInschrijvingen: Record<string, { vnaam: string; trainingen: string[]; email: string }>[] = [];
  for (const inschrijving of inschrijvingen) {
    const klant_id = inschrijving.klant.id;
    const klant = await getKlantById(klant_id);
    if (!klant) continue;
    const klantFoundInList = orderedInschrijvingen.find((record) => Object.keys(record).includes(klant_id.toString()));
    if (!klantFoundInList) {
