...
const Index = () => {
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await getData<PaginatedResponse<PaginatedKlantHond>>(
  ...
  const rows = useMemo(() => {
    return apiData.data.map((klantHond) => {
      const naam = <Link href={`/admin/honden/${klantHond._id}`}>{klantHond.naam}</Link>;
      const ras = <Link href={`/admin/rassen/${klantHond.ras}`}>{klantHond.ras}</Link>;
      const klant = (
        <Link href={`/admin/klanten/${klantHond.klant._id}`}>{klantHond.klant.naam}</Link>
      );
  ...
  const onPaginationClick = async (api?: string) => {
    if (!api) return;
    const { data, error } = await getData<PaginatedResponse<PaginatedKlantHond>>(api);
    if (data) setApiData(data);
